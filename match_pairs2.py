...
if __name__ == '__main__':
    ...
    index = 0
    matching_trt = TRTModule()
    with torch.no_grad():
        with torchprof.Profile(matching_trt, use_cuda=True, profile_memory=True) as prof:
            for i, pair in enumerate(pairs):
                ...
                if do_match:
                    ...
                    #print(type(inp0))
                    joined = torch.cat((inp0,inp1))
                    joined = torch.stack([joined]).cuda()
                    #ONNX_FILE_PATH = 'sp+sg.onnx'
                    #(matching, joined, ONNX_FILE_PATH, target_opset={'': 15, 'ai.onnx.ml': 2})
                    if not index:
                        matching_trt = torch2trt(matching.eval(),[joined],log_level=trt.Logger.INFO,fp16_mode=True)
                        index += 1
                    #pred = matching_trt({'image0': inp0, 'image1': inp1})
                    pred = matching_trt(joined)
                    continue
...
# Trace(path=('AlexNet', 'features', '0'), leaf=True, module=Conv2d(3, 64, kernel_size=(11, 11), stride=(4, 4), padding=(2, 2)))
    ...
    #result += event_lists_dict[trace[2].path][0]
